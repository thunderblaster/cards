...
io.on('connection', function (socket) { // The socket.io connection is first called when a user requests to join a room
    ...
    socket.on('selected', function (msg) { // Player selects a card to submit to Card Czar
        if(socket.room) { // If they have an old tab open and are trying to play a card in a room the server doesn't recognize, do not attempt to do anything with it.
        ...
        } else { // If the player didn't have a room associated with them
    ...
    socket.on('drawfivecards', function () { // This is identical except there's an array. And it used to deal 5 cards but I changed that
        if(socket.room) {                    // without changing the name of the function. Judge away. I deserve it.
            ...
            io.to(socket.id).emit('whoareyou'); // See above, tell the client they have a stale session if we don't see a room assigned to them
        }
    ...
    socket.on('winningcard', function(msg) { // Triggers when the card czar selects a winning card
        if(socket.room) { // Detect people with invalid sessions
        ...
        } else { // if invalid session
            io.to(socket.id).emit('whoareyou'); // tell the client
        }
